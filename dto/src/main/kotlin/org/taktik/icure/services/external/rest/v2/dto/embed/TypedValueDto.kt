...
data class TypedValueDto(
	...
	Serializable {
	companion object {
		fun <T> withValue(value: T?): TypedValueDto? = value?.let {
			withTypeAndValue(
				when (it) {
		...
		fun <T> withTypeAndValue(
			type: TypedValuesTypeDto,
			value: T?,
		): TypedValueDto? = value?.let {
			when (type) {
				TypedValuesTypeDto.BOOLEAN ->
					if (it is Boolean) {
				...
				TypedValuesTypeDto.STRING, TypedValuesTypeDto.JSON, TypedValuesTypeDto.CLOB ->
					if (it is String) {
