...
data class ServiceDto(
	...
	@get:Schema(description = "Id of the contact during which the service is provided") val contactId: String? = null,
	@get:Schema(
	...
	@get:Schema(description = "The secret patient key, encrypted in the patient document, in clear here.") val secretForeignKeys: Set<String>? =
		emptySet(), // Only used when the ServiceDto is emitted outside of its contact
	@get:Schema(description = "The public patient key, encrypted here for separate Crypto Actors.") val cryptedForeignKeys: Map<String, Set<DelegationDto>> = emptyMap(), // Only used when the ServiceDto is emitted outside of its contact
	@get:Schema(description = "The delegations giving access to connected healthcare information.") val delegations: Map<String, Set<DelegationDto>> = emptyMap(), // Only used when the ServiceDto is emitted outside of its contact
	@get:Schema(
	...
	@get:Schema(description = "Used for sorting services inside an upper object (A contact, a transaction, a FHIR bundle, ...)") val index: Long? = null, // Used for sorting
	@get:Schema(description = "Information contained in the service. Content is localized, using ISO language code as key") val content: Map<String, ContentDto> = emptyMap(), // Localized, in the case when the service contains a document, the document id is the SerializableValue
	...
	@get:Schema(description = "The date (YYYYMMDDhhmmss) of the start of the Service") val openingDate: Long? = null, // YYYYMMDDHHMMSS if unknown, 00, ex:20010800000000. Note that to avoid all confusion: 2015/01/02 00:00:00 is encoded as 20140101235960.
	@get:Schema(description = "The date (YYYYMMDDhhmmss) marking the end of the Service") val closingDate: Long? = null, // YYYYMMDDHHMMSS if unknown, 00, ex:20010800000000. Note that to avoid all confusion: 2015/01/02 00:00:00 is encoded as 20140101235960.
	@get:Schema(description = "Id of the form used during the Service") val formId: String? = null, // Used to group logically related services
	...
	@get:Schema(description = "Text, comments on the Service provided") val comment: String? = null,
	val status: Int? = null, // bit 0: active/inactive, bit 1: relevant/irrelevant, bit2 : present/absent, ex: 0 = active,relevant and present
	@get:Schema(description = "List of invoicing codes") val invoicingCodes: Set<String> = emptySet(),
	@get:Schema(description = "Comments - Notes recorded by a HCP about this service") val notes: List<AnnotationDto> = emptyList(),
	@get:Schema(description = "Links towards related services (possibly in other contacts)") val qualifiedLinks: Map<LinkQualificationDto, Map<String, String>> = emptyMap(), // Links towards related services (possibly in other contacts)
