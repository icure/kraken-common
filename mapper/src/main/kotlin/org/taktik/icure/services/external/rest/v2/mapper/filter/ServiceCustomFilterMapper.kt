...
import org.taktik.icure.entities.filters.ServiceByKeysCustomFilter
import org.taktik.icure.entities.filters.ServiceByRangeCustomFilter
...
/**
 * Maps the generic [CustomFilterDto] payload received by the Service custom-filtering
 * endpoint to the concrete [ServiceByKeysCustomFilter]/[ServiceByRangeCustomFilter] domain filter.
 */
@Service
class ServiceCustomFilterV2Mapper(
	...
	fun map(customFilterDto: CustomFilterDto): AbstractCustomFilter = when (customFilterDto) {
		is GenericByKeysCustomFilterDto -> ServiceByKeysCustomFilter(
		...
		is GenericByRangeCustomFilterDto -> ServiceByRangeCustomFilter(
			viewName = customFilterDto.viewName,
			startKey = keyComponentMapper.mapStartKey(customFilterDto.startKey),
			startDocumentId = customFilterDto.startDocumentId,
			limit = customFilterDto.limit,
