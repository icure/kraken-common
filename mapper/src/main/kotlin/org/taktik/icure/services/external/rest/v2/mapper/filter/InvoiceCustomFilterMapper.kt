...
import org.taktik.icure.entities.filters.InvoiceByKeysCustomFilter
import org.taktik.icure.entities.filters.InvoiceByRangeCustomFilter
...
/**
 * Maps the generic [CustomFilterDto] payload received by the Invoice custom-filtering
 * endpoint to the concrete [InvoiceByKeysCustomFilter]/[InvoiceByRangeCustomFilter] domain filter.
 */
@Service
class InvoiceCustomFilterV2Mapper(
	...
	fun map(customFilterDto: CustomFilterDto): AbstractCustomFilter = when (customFilterDto) {
		is GenericByKeysCustomFilterDto -> InvoiceByKeysCustomFilter(
		...
		is GenericByRangeCustomFilterDto -> InvoiceByRangeCustomFilter(
			viewName = customFilterDto.viewName,
			startKey = keyComponentMapper.mapStartKey(customFilterDto.startKey),
			startDocumentId = customFilterDto.startDocumentId,
			limit = customFilterDto.limit,
