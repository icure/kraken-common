...
import org.taktik.icure.entities.filters.PatientByKeysCustomFilter
import org.taktik.icure.entities.filters.PatientByRangeCustomFilter
...
/**
 * Maps the generic [CustomFilterDto] payload received by the Patient custom-filtering
 * endpoint to the concrete [PatientByKeysCustomFilter]/[PatientByRangeCustomFilter] domain filter.
 */
@Service
class PatientCustomFilterV2Mapper(
	...
	fun map(customFilterDto: CustomFilterDto): AbstractCustomFilter = when (customFilterDto) {
		is GenericByKeysCustomFilterDto -> PatientByKeysCustomFilter(
		...
		is GenericByRangeCustomFilterDto -> PatientByRangeCustomFilter(
			viewName = customFilterDto.viewName,
			startKey = keyComponentMapper.mapStartKey(customFilterDto.startKey),
			startDocumentId = customFilterDto.startDocumentId,
			limit = customFilterDto.limit,
