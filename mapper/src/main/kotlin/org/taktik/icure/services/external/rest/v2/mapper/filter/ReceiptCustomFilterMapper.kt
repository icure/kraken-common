...
import org.taktik.icure.entities.filters.ReceiptByKeysCustomFilter
import org.taktik.icure.entities.filters.ReceiptByRangeCustomFilter
...
/**
 * Maps the generic [CustomFilterDto] payload received by the Receipt custom-filtering
 * endpoint to the concrete [ReceiptByKeysCustomFilter]/[ReceiptByRangeCustomFilter] domain filter.
 */
@Service
class ReceiptCustomFilterV2Mapper(
	...
	fun map(customFilterDto: CustomFilterDto): AbstractCustomFilter = when (customFilterDto) {
		is GenericByKeysCustomFilterDto -> ReceiptByKeysCustomFilter(
		...
		is GenericByRangeCustomFilterDto -> ReceiptByRangeCustomFilter(
			viewName = customFilterDto.viewName,
			startKey = keyComponentMapper.mapStartKey(customFilterDto.startKey),
			startDocumentId = customFilterDto.startDocumentId,
			limit = customFilterDto.limit,
