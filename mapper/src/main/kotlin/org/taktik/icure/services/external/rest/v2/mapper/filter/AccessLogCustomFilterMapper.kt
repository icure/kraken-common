...
import org.taktik.icure.entities.filters.AccessLogByKeysCustomFilter
import org.taktik.icure.entities.filters.AccessLogByRangeCustomFilter
...
/**
 * Maps the generic [CustomFilterDto] payload received by the AccessLog custom-filtering
 * endpoint to the concrete [AccessLogByKeysCustomFilter]/[AccessLogByRangeCustomFilter] domain filter.
 */
@Service
class AccessLogCustomFilterV2Mapper(
	...
	fun map(customFilterDto: CustomFilterDto): AbstractCustomFilter = when (customFilterDto) {
		is GenericByKeysCustomFilterDto -> AccessLogByKeysCustomFilter(
		...
		is GenericByRangeCustomFilterDto -> AccessLogByRangeCustomFilter(
			viewName = customFilterDto.viewName,
			startKey = keyComponentMapper.mapStartKey(customFilterDto.startKey),
			startDocumentId = customFilterDto.startDocumentId,
			limit = customFilterDto.limit,
