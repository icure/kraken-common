...
abstract class PermissionItemV2Mapper {
	...
	fun map(permissionItemDto: PermissionItemDto): PermissionItem = when (permissionItemDto) {
		is AlwaysPermissionItemDto -> map(permissionItemDto)
		else -> throw IllegalArgumentException("Unsupported permission class")
	}

	fun map(permissionItem: PermissionItem): PermissionItemDto = when (permissionItem) {
