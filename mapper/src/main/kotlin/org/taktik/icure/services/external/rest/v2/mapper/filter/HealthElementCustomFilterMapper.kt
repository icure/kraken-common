...
import org.taktik.icure.entities.filters.HealthElementByKeysCustomFilter
import org.taktik.icure.entities.filters.HealthElementByRangeCustomFilter
...
/**
 * Maps the generic [CustomFilterDto] payload received by the HealthElement custom-filtering
 * endpoint to the concrete [HealthElementByKeysCustomFilter]/[HealthElementByRangeCustomFilter] domain filter.
 */
@Service
class HealthElementCustomFilterV2Mapper(
	...
	fun map(customFilterDto: CustomFilterDto): AbstractCustomFilter = when (customFilterDto) {
		is GenericByKeysCustomFilterDto -> HealthElementByKeysCustomFilter(
		...
		is GenericByRangeCustomFilterDto -> HealthElementByRangeCustomFilter(
			viewName = customFilterDto.viewName,
			startKey = keyComponentMapper.mapStartKey(customFilterDto.startKey),
			startDocumentId = customFilterDto.startDocumentId,
			limit = customFilterDto.limit,
