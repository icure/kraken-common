...
import org.taktik.icure.customentities.config.VersionedCustomEntitiesConfiguration
...
fun CustomEntityConfigResolutionContext.resolveRequiredObjectReference(
	reference: String
): ObjectDefinition = checkNotNull(resolveObjectReference(reference)) {
...
fun CustomEntityConfigResolutionContext.resolveRequiredEnumReference(
	reference: String
): EnumDefinition = checkNotNull(resolveEnumReference(reference)) {
	"Object definition for reference `$reference` not found"
}
