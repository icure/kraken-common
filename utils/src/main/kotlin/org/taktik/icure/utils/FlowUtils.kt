 ...
 */
fun <T : Any> Flow<T>.injectReactorContext(): Flux<T> = Mono.deferContextual { Mono.just(it) }.flatMapMany { ctxView ->
...
suspend fun Flow<ByteBuffer>.toInputStream(): InputStream = withContext(IO) {
	...
	object : InputStream() {
		...
		override fun read(): Int = if (buffers[idx].hasRemaining()) {
			(buffers[idx].get().toUInt() and 0xffu).toInt()
		} else {
...
fun <T> Flow<T>.distinctIf(condition: Boolean) = if (condition) {
	this.distinct()
} else {
	this
}
 ...
 */
suspend fun Flow<DataBuffer>.toByteArray(releaseBuffers: Boolean): ByteArray = ByteArrayOutputStream().use { os ->
	collect {
...
suspend fun Flow<ByteBuffer>.toByteArray(): ByteArray = ByteArrayOutputStream().use { os ->
	collect { it.writeTo(os) }
	os.toByteArray()
}

private fun ByteBuffer.writeTo(os: OutputStream): Unit = if (hasArray() && hasRemaining()) {
