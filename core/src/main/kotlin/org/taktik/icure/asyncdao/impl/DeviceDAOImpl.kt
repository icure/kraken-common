...
class DeviceDAOImpl(
	...
	override fun listDeviceIdsByResponsible(
		datastoreInformation: IDatastoreInformation,
	...
	override suspend fun getDevice(
		datastoreInformation: IDatastoreInformation,
		deviceId: String,
	): Device? = get(datastoreInformation, deviceId)
		...
		return result.fold(emptyMap()) { acc, (key, value) ->
			if (key != null && value != null) {
				acc +
					(
						value[0] to
							(acc[value[0]] ?: emptyMap()).let {
								it +
									(
										value[1].let { v -> v.substring((v.length - 32).coerceAtLeast(0)) }
