...
internal class FormDAOImpl(
	...
	@Views(
		...
		View(
			...
			map = "classpath:js/form/By_data_owner_patientfk_map.js",
			secondaryPartition = DATA_OWNER_PARTITION,
		),
	)
	override fun listFormsByHcPartyPatient(
		...
		emitAll(
			client
				.interleave<Array<String>, String, Form>(viewQueries, compareBy({ it[0] }, { it[1] }))
	...
	@View(
		...
		map = "classpath:js/form/By_hcparty_patientfk_date_map.js",
	...
	override fun listFormsByHcPartyAndParentId(
		datastoreInformation: IDatastoreInformation,
		searchKeys: Set<String>,
		formId: String,
	) = flow {
	...
	@Views(
		View(
			name = "by_logicalUuid",
			map = "function(doc) { if (doc.java_type == 'org.taktik.icure.entities.Form' && !doc.deleted && doc.logicalUuid) emit( doc.logicalUuid, doc._id )}",
		),
	...
	@Views(
		View(
			name = "by_uniqueId",
			map = "function(doc) { if (doc.java_type == 'org.taktik.icure.entities.Form' && !doc.deleted && doc.uniqueId) emit( doc.uniqueId, doc._id )}",
		),
