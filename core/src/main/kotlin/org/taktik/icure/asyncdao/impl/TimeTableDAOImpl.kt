...
@View(
	name = "all",
	map = "function(doc) { if (doc.java_type == 'org.taktik.icure.entities.TimeTable' && !doc.deleted) emit( null, doc._id )}",
)
class TimeTableDAOImpl(
...
) : GenericDAOImpl<TimeTable>(
	entityClass = TimeTable::class.java,
	...
	override fun listTimeTablesByAgendaId(
		datastoreInformation: IDatastoreInformation,
		agendaId: String,
	) = flow {
		val client = couchDbDispatcher.getClient(datastoreInformation)
		val viewQuery =
			createQuery(datastoreInformation, "by_agenda")
				.startKey(agendaId)
				.endKey(agendaId)
				.includeDocs(true)
	...
	override fun listTimeTableIdsByAgendaId(
		datastoreInformation: IDatastoreInformation,
		agendaId: String,
	) = flow {
		val client = couchDbDispatcher.getClient(datastoreInformation)
	...
	override fun listTimeTablesByAgendaIds(
		datastoreInformation: IDatastoreInformation,
		agendaIds: Collection<String>,
	) = flow {
		val client = couchDbDispatcher.getClient(datastoreInformation)
		val viewQuery =
			createQuery(datastoreInformation, "by_agenda")
	...
	private fun listTimeTablesByStartDateAndAgendaId(
		...
		val viewQuery =
			createQuery(datastoreInformation, "by_agenda_and_startdate")
	...
	override fun listTimeTablesByPeriodAndAgendaId(
		...
		agendaId: String,
		descending: Boolean,
	): Flow<TimeTable> = listTimeTablesByStartDateAndAgendaId(datastoreInformation, agendaId, descending).filter {
	...
	override fun listTimeTableIdsByPeriodAndAgendaId(
		...
		agendaId: String,
		descending: Boolean,
	): Flow<String> = listTimeTablesByPeriodAndAgendaId(datastoreInformation, startDate, endDate, agendaId, descending).map { it.id }

	override fun listTimeTablesByPeriodAndAgendaIds(
		datastoreInformation: IDatastoreInformation,
		startDate: Long?,
		endDate: Long?,
		agendaIds: Collection<String>,
	): Flow<TimeTable> = listTimeTablesByAgendaIds(
