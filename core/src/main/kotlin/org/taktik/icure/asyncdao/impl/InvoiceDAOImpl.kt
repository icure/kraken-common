...
@View(
	name = "all",
	map = "function(doc) { if (doc.java_type == 'org.taktik.icure.entities.Invoice' && !doc.deleted) emit( null, doc._id )}",
)
class InvoiceDAOImpl(
	...
	override fun findInvoicesByHcParty(
		...
		emitAll(
			client.interleave<ComplexKey, String, Invoice>(
				viewQueries,
				compareBy({
	...
	@Views(
		...
		View(
			name = "by_data_owner_contact",
			map = "classpath:js/invoice/By_data_owner_contact_map.js",
			secondaryPartition = DATA_OWNER_PARTITION,
		),
	)
	override fun listInvoicesByHcPartyAndContacts(
		datastoreInformation: IDatastoreInformation,
		searchKeys: Set<String>,
		contactId: Set<String>,
	) = flow {
		val client = couchDbDispatcher.getClient(datastoreInformation)
	...
	@View(
		name = "by_data_owner_decision_reference_map",
		map = "classpath:js/invoice/By_data_owner_decision_reference_map.js",
		secondaryPartition = DATA_OWNER_PARTITION,
	)
			...
			client
				.queryView<ComplexKey, String>(
					createQuery(
						...
						secondaryPartition = DATA_OWNER_PARTITION,
					).includeDocs(false).keys(searchKeys.map { key -> ComplexKey.of(key, decisionReference) }).reduce(false),
				).map { it.id },
		)
	}
	...
	@Views(
		...
		View(
			name = "by_data_owner_reference",
			map = "classpath:js/invoice/By_data_owner_reference_map.js",
			secondaryPartition = DATA_OWNER_PARTITION,
		),
	)
	override fun listInvoicesByHcPartyAndReferences(
		datastoreInformation: IDatastoreInformation,
		hcParty: String,
		invoiceReferences: Set<String>?,
	) = flow {
		val client = couchDbDispatcher.getClient(datastoreInformation)
	...
	@Views(
		...
		View(
			name = "by_data_owner_groupid",
			map = "classpath:js/invoice/By_data_owner_groupid_map.js",
			secondaryPartition = DATA_OWNER_PARTITION,
		),
	)
	...
	@Views(
		...
		View(
			name = "by_data_owner_recipient",
			map = "classpath:js/invoice/By_data_owner_recipient_map.js",
			secondaryPartition = DATA_OWNER_PARTITION,
		),
	)
	override fun listInvoicesByHcPartyAndRecipientIds(
		datastoreInformation: IDatastoreInformation,
		searchKeys: Set<String>,
	...
	@Views(
		...
		View(
			name = "by_data_owner_patientfk",
			map = "classpath:js/invoice/By_data_owner_patientfk_map.js",
			secondaryPartition = DATA_OWNER_PARTITION,
		),
	)
	override fun listInvoicesByHcPartyAndPatientSfks(
		datastoreInformation: IDatastoreInformation,
		searchKeys: Set<String>,
	...
	@View(
		name = "by_hcparty_patientfk_date",
		map = "classpath:js/invoice/By_hcparty_patientfk_date_map.js",
	...
	@Views(
		...
		View(
			name = "by_data_owner_recipient_unsent",
			map = "classpath:js/invoice/By_data_owner_recipient_unsent_map.js",
			secondaryPartition = DATA_OWNER_PARTITION,
		),
	)
	...
	@Views(
		...
		View(
			name = "by_data_owner_patientfk_unsent",
			map = "classpath:js/invoice/By_data_owner_patientfk_unsent_map.js",
			secondaryPartition = DATA_OWNER_PARTITION,
		),
	)
	override fun listInvoicesByHcPartyAndPatientFkUnsent(
		datastoreInformation: IDatastoreInformation,
		searchKeys: Set<String>,
		secretPatientKeys: Set<String>,
	) = flow {
	...
	@Views(
		View(
			name = "by_hcparty_sentmediumtype_invoicetype_sent_date",
			map = "classpath:js/invoice/By_hcparty_sentmediumtype_invoicetype_sent_date.js",
		),
		View(
			name = "by_data_owner_sentmediumtype_invoicetype_sent_date",
			map = "classpath:js/invoice/By_data_owner_sentmediumtype_invoicetype_sent_date.js",
			secondaryPartition = DATA_OWNER_PARTITION,
		),
	)
	...
	@Views(
		...
		View(
			name = "by_data_owner_sending_mode_status_date",
			map = "classpath:js/invoice/By_data_owner_sending_mode_status_date.js",
			secondaryPartition = DATA_OWNER_PARTITION,
		),
	)
	...
	override fun listInvoicesByServiceIds(
		datastoreInformation: IDatastoreInformation,
		...
		emitAll(
			client
				.queryViewIncludeDocs<String, String, Invoice>(
					createQuery(datastoreInformation, "by_serviceid").includeDocs(true).keys(serviceIds),
				).map {
					it.doc
				},
		)
	...
	override fun listInvoicesHcpsByStatus(
		datastoreInformation: IDatastoreInformation,
		status: String,
		from: Long?,
		to: Long?,
				...
				.map { hcpId ->
					client
						.queryView<Array<String>, String>(
							createQuery(datastoreInformation, "by_status_hcps_sentdate")
								.includeDocs(false)
	...
	@View(
		...
		map = "classpath:js/invoice/Tarification_by_data_owner_code.js",
		reduce = "_count",
		secondaryPartition = DATA_OWNER_PARTITION,
	)
	override fun listInvoiceIdsByTarificationsAndCode(
		datastoreInformation: IDatastoreInformation,
		hcPartyId: String,
		codeCode: String?,
		startValueDate: Long?,
	...
	override fun listTarificationsFrequencies(
		datastoreInformation: IDatastoreInformation,
		hcPartyId: String,
	) = flow {
		val client = couchDbDispatcher.getClient(datastoreInformation)
		...
		emitAll(
			client.queryView<ComplexKey, Long>(
				createQuery(
					...
					"tarification_by_data_owner",
					DATA_OWNER_PARTITION,
				).startKey(from).endKey(to).includeDocs(false).reduce(true).group(true).groupLevel(2),
			),
		)
	}
