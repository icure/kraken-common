...
open class MessageDAOImpl(
	...
	override fun listMessagesByFromAddress(
		...
		val viewQueries = createPagedQueries(
			...
			legacyViewQueries = listOf(
				"by_hcparty_from_address".main(),
				"by_data_owner_from_address" to DATA_OWNER_PARTITION,
			),
			configurationViews = listOf("by_all_delegates_from_address"),
			startKey = startKey,
			endKey = endKey,
	...
	override fun findMessagesByToAddress(
		...
		val viewQueries = createPagedQueries(
			client = client,
			legacyViewQueries = listOf(
				"by_hcparty_to_address".main(),
				"by_data_owner_to_address" to DATA_OWNER_PARTITION,
			),
			configurationViews = listOf("by_all_delegates_to_address"),
			startKey = startKey,
			endKey = endKey,
			pagination = paginationOffset,
			descending = reverse,
	...
	override fun findMessagesByTransportGuid(
		...
		val startKey = getGuidPrefix(transportGuid)?.let {
			ComplexKey.of(partyId, it)
		} ?: ComplexKey.of(partyId, transportGuid)
		val endKey = getGuidPrefix(transportGuid)?.let {
			ComplexKey.of(partyId, it + "\ufff0")
		} ?: ComplexKey.of(partyId, transportGuid)
		...
		val viewQueries = createPagedQueries(
			client = client,
			legacyViewQueries = listOf(
				"by_hcparty_transport_guid".main(),
				"by_data_owner_transport_guid" to DATA_OWNER_PARTITION,
			),
			configurationViews = listOf("by_all_delegates_transport_guid"),
			startKey = startKey,
			endKey = endKey,
			pagination = paginationOffset,
			descending = false,
	...
	override fun findMessagesByTransportGuidReceived(
		...
		val startKey = getGuidPrefix(transportGuid)?.let {
			ComplexKey.of(partyId, it, null)
		} ?: ComplexKey.of(partyId, transportGuid, null)
		val endKey = getGuidPrefix(transportGuid)?.let {
			ComplexKey.of(partyId, it + "\ufff0", ComplexKey.emptyObject())
		} ?: ComplexKey.of(partyId, transportGuid, ComplexKey.emptyObject())

		val viewQueries = createPagedQueries(
			client = client,
			legacyViewQueries = listOf(
				"by_hcparty_transport_guid_received".main(),
				"by_data_owner_transport_guid_received" to DATA_OWNER_PARTITION,
			),
			configurationViews = listOf("by_all_delegates_transport_guid_received"),
			startKey = startKey,
			endKey = endKey,
			pagination = paginationOffset,
			descending = false,
	...
	override fun listMessageIdsByTransportGuidReceived(
		...
		val startKey = getGuidPrefix(transportGuid)?.let {
			ComplexKey.of(dataOwnerId, it, null)
		} ?: ComplexKey.of(dataOwnerId, transportGuid, null)
		val endKey = getGuidPrefix(transportGuid)?.let {
			ComplexKey.of(dataOwnerId, it + "\ufff0", ComplexKey.emptyObject())
		} ?: ComplexKey.of(dataOwnerId, transportGuid, ComplexKey.emptyObject())
	...
	override fun findMessagesByTransportGuidAndSentDate(datastoreInformation: IDatastoreInformation, partyId: String, transportGuid: String, fromDate: Long, toDate: Long, paginationOffset: PaginationOffset<ComplexKey>) = flow {
		...
		val viewQueries = createPagedQueries(
			client = client,
			legacyViewQueries = listOf(
				"by_hcparty_transport_guid_sent_date".main(),
				"by_data_owner_transport_guid_sent_date" to DATA_OWNER_PARTITION,
			),
			configurationViews = listOf("by_all_delegates_transport_guid_sent_date"),
			startKey = startKey,
			endKey = endKey,
			pagination = paginationOffset,
			descending = false,
	...
	override fun listMessagesByHcPartyAndPatient(datastoreInformation: IDatastoreInformation, searchKeys: Set<String>, secretPatientKeys: List<String>) = flow {
		...
		emitAll(
			client
				.interleave<ComplexKey, String, Message>(viewQueries, compareBy({ it.components[0] as String }, { it.components[1] as String }))
	...
	override fun listMessageIdsByDataOwnerPatientSentDate(
		...
		legacyViews = listOf("by_hcparty_patientfk_sent" to MAURICE_PARTITION, "by_data_owner_patientfk" to DATA_OWNER_PARTITION),
		configurationViews = listOf("by_all_delegates_patientfk"),
	...
	override fun getChildren(datastoreInformation: IDatastoreInformation, messageId: String) = flow {
		...
		emitAll(
			client
				.queryViewIncludeDocs<String, Int, Message>(
					createQuery(
						client = client,
						legacyView = "by_parent_id".main(),
						configurationView = "by_parent_id"
					).includeDocs(true).key(messageId)
				).map { it.doc }
		)
	}
	...
	override fun getMessagesChildren(datastoreInformation: IDatastoreInformation, parentIds: List<String>) = flow {
		...
		val query = createQuery(
			...
			legacyView = "by_parent_id".main(),
			configurationView = "by_parent_id"
		).includeDocs(true).keys(parentIds)
	...
	override fun listMessageIdsByParents(datastoreInformation: IDatastoreInformation, parentIds: List<String>): Flow<String> = flow {
		...
		val query = createQuery(
			...
			legacyView = "by_parent_id".main(),
			configurationView = "by_parent_id"
		).includeDocs(false).keys(parentIds)
	...
	override fun listMessagesByInvoiceIds(datastoreInformation: IDatastoreInformation, invoiceIds: Set<String>) = flow {
		val client = couchDbDispatcher.getClient(datastoreInformation)
		emitAll(
			client.queryViewIncludeDocs<String, Int, Message>(
				createQuery(
					client = client,
					legacyView = "by_invoice_id".main(),
					configurationView = "by_invoice_id"
				).includeDocs(true).keys(invoiceIds)
			).map { it.doc }
		)
	}
	...
	override fun listMessageIdsByInvoiceIds(datastoreInformation: IDatastoreInformation, invoiceIds: Set<String>): Flow<String> = flow {
		val client = couchDbDispatcher.getClient(datastoreInformation)
		emitAll(
			client.queryView<String, Int>(
				createQuery(
					client = client,
					legacyView = "by_invoice_id".main(),
					configurationView = "by_invoice_id"
				).includeDocs(false).keys(invoiceIds)
			).map { it.id }
		)
	}
	...
	@View(
		name = "conflicts",
		map = "function(doc) { if (doc.java_type == 'org.taktik.icure.entities.Message' && !doc.deleted && doc._conflicts) emit(doc._id) }"
	)
	override fun listConflicts(datastoreInformation: IDatastoreInformation) =
		doListConflicts<Message>(datastoreInformation, "conflicts", null)

	override fun listIdsOfEntitiesWithConflicts(datastoreInformation: IDatastoreInformation): Flow<String> =
		doListIdsOfEntitiesWithConflicts<Message>(datastoreInformation, "conflicts", null)
	...
	private fun listMessageIdsByDataOwnerTagOrCode(
		...
		val query = createQuery(
			client = client,
			legacyView = legacyView to MAURICE_PARTITION,
	...
	override fun listMessageIdsByDataOwnerTag(
		...
		tagType: String,
		tagCode: String?
	): Flow<String> = listMessageIdsByDataOwnerTagOrCode(
		legacyView = "by_data_owner_tag",
