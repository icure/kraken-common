...
abstract class DataAttachmentModificationLogicImpl<T : HasDataAttachments<T>>(
	...
	override fun ensureValidAttachmentChanges(
		currEntity: T,
		newEntity: T,
		lenientKeys: Set<String>,
	): T {
	...
	override suspend fun updateAttachments(
		...
		if (expectedRev != null &&
			currEntity.rev != expectedRev
		) {
			throw ConflictRequestException("Rev mismatch for $entityId: expected $expectedRev")
		}
		validateDeleteRequests(currEntity, changes)
		val dataAttachmentUpdateInfos =
			changes.mapValues { (key, change) ->
		 ...
		 */
		val updatedDataAttachmentsInfo =
			(currEntity.dataAttachments.keys + dataAttachmentUpdateInfos.keys)
				.flatMap { key ->
		...
		val updatedDeletedAttachmentsInfo =
			System.currentTimeMillis().let { now ->
				currEntity.deletedAttachments +
					dataAttachmentUpdateInfos.mapNotNull { (key, result) ->
		...
		val newAttachments =
			updatedDataAttachmentsInfo.values.mapNotNull { it.couchDbAttachmentId }.associateWith {
	...
	private fun validateDeleteRequests(
		currEntity: T,
		changes: Map<String, DataAttachmentChange>,
	) {
	...
	private suspend fun prepareCreateOrUpdateAttachmentTask(
		...
		val (newAttachment, uploadTask) =
			doPreStoreAndPrepareAttachmentInfoAndTasks(
				entity,
				change,
	...
	private suspend fun doPreStoreAndPrepareAttachmentInfoAndTasks(
		...
		change: DataAttachmentChange.CreateOrUpdate,
		utis: List<String>,
	): Pair<DataAttachment, AttachmentTask?> = if (change.size < objectStorageProperties.sizeLimit) {
		...
		if (entity.dataAttachments.values.any { it.couchDbAttachmentId == attachmentId }) {
			throw IllegalArgumentException(
	...
	): String = tasks.filterIsInstance<AttachmentTask.UploadCouchDb>().fold(initialRev) { latestRev, task ->
	...
	private sealed interface AttachmentTask {
		...
		class PreStoreAndUploadObjectStorage(
			val attachmentId: String,
			val dataFlow: Flow<DataBuffer>,
			val size: Long,
		) : AttachmentTask
...
) : DocumentDataAttachmentModificationLogic,
	DataAttachmentModificationLogic<Document> by object : DataAttachmentModificationLogicImpl<Document>(
		dao,
		icureObjectStorage,
