...
class PatientByHcPartyAndSsinFilter(
	...
	override val configurationViews = listOf(ConfigurationView("Patient", "by_all_delegates_ssin"))
	...
	override fun resolve(
		...
		try {
			val hcpId = requireNotNull(filter.healthcarePartyId ?: sessionLogic.getCurrentDataOwnerIdOrNull()) {
				"A PatientByHcPartyAndSsinFilter must either provide an explicit dataOwnerId or must be used by a data owner user"
			}
			emitAll(
