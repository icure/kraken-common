...
class PatientByHcPartyAndExternalIdFilter(
	...
	override val configurationViews = listOf(ConfigurationView("Patient", "by_all_delegates_externalid"))
	...
	override fun resolve(
		...
		try {
			val hcpId = requireNotNull(filter.healthcarePartyId ?: sessionLogic.getCurrentDataOwnerIdOrNull()) {
				"A PatientByHcPartyAndExternalIdFilter must either provide an explicit dataOwnerId or must be used by a data owner user"
			}
