...
open class CodeLogicImpl(
    ...
    override suspend fun isValid(code: Code, ofType: String?): Boolean =
        isValid(CodeStub(id = code.id, code = code.code, type = code.type, version = code.version), ofType)

    override suspend fun isValid(code: CodeStub, ofType: String?): Boolean {
        val datastoreInformation = getInstanceAndGroup()
        val codeType = ofType ?: code.type
        return if (codeType != null && code.code != null) codeDAO.isValid(
            datastoreInformation,
            codeType,
            code.code!!,
            code.version
    ...
    protected fun doSolveConflicts(
        ...
        val flow = ids?.asFlow()?.mapNotNull { codeDAO.get(datastoreInformation, it, Option.CONFLICTS) }
