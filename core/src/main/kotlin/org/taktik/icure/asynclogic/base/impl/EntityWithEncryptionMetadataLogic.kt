...
abstract class EntityWithEncryptionMetadataLogic<E, D>(
	...
	private val helper =
		EntityWithEncryptionMetadataLogicHelper<E, D> {
	 ...
	 */
	suspend fun getAllSearchKeysIfCurrentDataOwner(dataOwnerId: String): Set<String> = sessionLogic.getAllSearchKeysIfCurrentDataOwner(dataOwnerId)
	...
	override fun bulkShareOrUpdateMetadata(requests: BulkShareOrUpdateMetadataParams): Flow<EntityBulkShareResult<E>> = flow {
		val datastoreInfo = datastoreInstanceProvider.getInstanceAndGroup()
		emitAll(
...
class EntityWithEncryptionMetadataLogicHelper<E, D>(
	...
	fun doBulkShareOrUpdateMetadata(
		...
		val validatedRequests =
			requests.requestsByEntityId.mapNotNull { (entityId, entityRequests) ->
				entities[entityId]?.let { verifyAndApplyShareOrUpdateRequest(entityRequests, it) }
			}
		val approvedChangesByEntityId =
			validatedRequests
				.mapNotNull {
					it.entityWithAppliedRequestsIds
						?.first
						?.id
						?.to(it)
				}.toMap()
		...
		validatedRequests.forEach {
			if (it.entityWithAppliedRequestsIds == null) {
				emit(
					EntityBulkShareResult(
						null,
						it.entityId,
	...
	private fun <T : HasEncryptionMetadata> getExchangeDataMapsToCreate(
		...
		.mapNotNull { validatedRequest ->
			...
			exchangeMaps +
				(
					requests.requestsByEntityId[entityId]
						?.requests
						?.filterKeys {
	...
	private fun verifyAndApplyShareOrUpdateRequest(
		...
		val appliedRequestsInfo =
			PartialRequestApplication(
				shareRequests,
				updateRequests,
				emptySet(),
				emptyMap(),
				...
				if (requestInfo.potentialParentDelegations.any {
						currentEntity.securityMetadata
							?.secureDelegations
							?.get(it)
	...
	private fun PartialRequestApplication<E>.rejectShareRequestsForExistingDelegations(): PartialRequestApplication<E> {
		val validRequests =
			currentEntity.securityMetadata?.secureDelegations?.keys?.let { existingDelegationKeys ->
	...
	private fun PartialRequestApplication<E>.createRootDelegations(): PartialRequestApplication<E> {
		val rootDelegationsRequests =
			remainingShareRequests.filter { (_, v) ->
		...
		return when (rootDelegationsRequests.size) {
			...
			1 -> {
				...
				val (newDelegationKey, updatedEntity) =
					updatedEntity.withNewSecureDelegation(
						request,
						emptySet(),
						AccessLevel.WRITE,
					)
				...
				throw IllegalStateException(
					"Data owners should not request many ROOT delegations for the same entity; this should have already been checked",
				)
		}
	}
	...
	private fun PartialRequestApplication<E>.createNonRootDelegations(): PartialRequestApplication<E> {
		...
		return if (potentialParentDelegations.isEmpty()) {
			copy(
				...
				rejectedRequests =
				rejectedRequests +
					remainingShareRequests.mapValues {
						RejectedShareOrMetadataUpdateRequest(
							400,
							false,
			...
			copy(
				...
				updatedEntity =
				remainingShareRequests.toList().fold(updatedEntity) { latestEntityUpdate, (_, request) ->
					latestEntityUpdate
						.withNewSecureDelegation(
							request,
	...
	private fun PartialRequestApplication<E>.applyUpdateRequests(): PartialRequestApplication<E> {
		...
		remainingUpdateRequest.forEach { (requestId, request) ->
			...
			if (delegation != null) {
				val updatedSecretIds =
					validateAndApplyUpdateRequests(
						delegation.secretIds,
						request.secretIds,
					)
				val updatedEncryptionKeys =
					validateAndApplyUpdateRequests(
						delegation.encryptionKeys,
						request.encryptionKeys,
					)
				val updatedOwningEntityIds =
					validateAndApplyUpdateRequests(
				...
				if (updatedSecretIds == null || updatedEncryptionKeys == null || updatedOwningEntityIds == null) {
					...
					updatedDelegations[canonicalHash] =
						delegation.copy(
	...
	private fun parentsFromAccessibleHashes(
		...
		return accessibleDelegationsHashes
			.filterNot {
	...
	private fun E.withNewSecureDelegation(
		...
		val newSecurityMetadata =
			securityMetadata?.let {
				it.copy(
