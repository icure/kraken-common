...
open class MessageLogicImpl(
	...
	override fun listMessagesByHCPartySecretPatientKeys(
		...
		emitAll(
			messageDAO.listMessagesByHcPartyAndPatient(datastoreInformation, getAllSearchKeysIfCurrentDataOwner(hcPartyId), secretPatientKeys),
		)
	}
		...
		emitAll(
			messageDAO
				.saveBulk(
					...
					messages
						.map { m ->
							if ((m.readStatus[userId]?.time ?: 0) < timeOrNow) {
								m.copy(
									readStatus =
									m.readStatus +
										(
	...
	override fun findMessagesByTransportGuidSentDate(
		...
		emitAll(
			messageDAO
				.findMessagesByTransportGuidAndSentDate(
					datastoreInformation,
					partyId,
					transportGuid,
					fromDate,
	...
	override suspend fun addDelegations(
		...
		return messageDAO.save(
			...
			message.copy(
				delegations =
				message.delegations + delegations.mapNotNull { d -> d.delegatedTo?.let { delegateTo -> delegateTo to setOf(d) } },
			),
		)
	}
	...
	override fun getMessagesByTransportGuids(
		hcpId: String,
		transportGuids: Set<String>,
	) = flow {
		val datastoreInformation = getInstanceAndGroup()
	...
	): Message = entity.copy(securityMetadata = updatedMetadata)
