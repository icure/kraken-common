...
data class RegimenItem(
	...
	override fun toString(): String {
		...
		var result = if (date != null) {
			String.format("the %s", df.format(date))
		} else if (dayNumber != null) {
			String.format("on day %d", dayNumber)
		} else if (weekday?.weekday?.code != null) {
			String.format("on %s", weekday.weekday.code)
		} else {
			null
		}
