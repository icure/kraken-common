...
data class ServiceByQualifiedLinkFilter(
	...
	ServiceByQualifiedLinkFilter {

	override val canBeUsedInWebsocket = true
	override val requiresSecurityPrecondition: Boolean = true
	override fun requestedDataOwnerIds(): Set<String> = emptySet()

	override fun matches(item: Service, searchKeyMatcher: (String, HasEncryptionMetadata) -> Boolean): Boolean = if (linkQualification != null) {
		item.qualifiedLinks[linkQualification]?.values?.any {
			linkValues.contains(it)
		} == true
	} else {
		item.qualifiedLinks.values.any { links ->
