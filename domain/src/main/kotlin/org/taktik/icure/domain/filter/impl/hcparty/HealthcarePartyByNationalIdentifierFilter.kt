...
data class HealthcarePartyByNationalIdentifierFilter(
...
) : AbstractFilter<HealthcareParty>, HealthcarePartyByNationalIdentifierFilter {
	...
	override fun matches(item: HealthcareParty, searchKeyMatcher: (String, HasEncryptionMetadata) -> Boolean): Boolean =
		listOfNotNull(item.ssin, item.nihii, item.cbe, item.ehp).any { it.startsWith(searchValue) }
}